...
def MLgenotype(rr, matchreferrors, alterrors, MLfile):

    g = [0, 1 , 2]                          ## ML for estimating Genotype calls 
    k = len(rr[2])
    a = 1 / pow(2,k)
    for x in g:
        if x == 0:
    ...
    MLgenotype = genotype0, genotype1, genotype2
    print(" g0: " + str(genotype0[0]), 
          "g1: " + str(genotype1[0]),
          "g2: " + str(genotype2[0]),
          max(MLgenotype), 
          file = MLfile)
...
def main(args):
    ...
    print ("(1) the base position, (2) the reference base, (3) the alt base,",file=MLfile )
    print ("(4) ML g0, (5) ML g1, (6) ML g2, (7) ML Genotype" "\n" ,file=MLfile )
    ...
    for rr in varrr:
        ...
        print(firstrefbase+rr[0], rr[1], rr[4], end='', file=MLfile)
        
        mlfunction = MLgenotype(rr, matchreferrors, alterrors, MLfile)
        mlfunction
        print(firstrefbase+rr[0],rr[1],rr[4], max(mlfunction))
    f.close()
    MLfile.close()
